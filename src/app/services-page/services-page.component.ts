...
export class ServicesPageComponent implements AfterViewInit {
  ...
  stackServices = [this.stackService1, this.stackService2, this.stackService3];
  index = 0;
  startIndex = true;
  lastIndex = false;
  ...
  chossePage(index: number): void {
    document.getElementById('service_mobile')?.scrollIntoView();
    document.getElementById('service_tablet')?.scrollIntoView();
    document.getElementById('service_pc')?.scrollIntoView();
    this.index = index;
    if (index == 0) {
